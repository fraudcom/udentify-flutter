...
/// Real-time feedback directive reported via the `onFaceDirectiveChanged`
/// callback during face recognition/liveness. Native source:
/// `io.udentify.android.face.model.FaceDirective` on Android,
/// `UdentifyFACE.FaceDirective` on iOS. The two platforms' enums differ
/// slightly (Android has `eyesClosed`, iOS has `faceNotFound` instead), so
/// this combines both sets rather than picking one.
enum FaceDirective {
  faceOk,
  faceOffCenter,
  faceIsAngled,
  ...
  faceIsTooSmall,
  headIsAngled,
  eyesClosed, // Android only
  maskDetected,
  tooManyFaces,
  faceNotFound, // iOS only
  unknown;

  static FaceDirective fromString(String? value) {
    for (final directive in FaceDirective.values) {
      if (directive.name == value) return directive;
    }
    return FaceDirective.unknown;
  }
}

/// Real-time feedback directive from the ActiveLivenessOperator interface
/// (`io.udentify.android.face.model.ActiveLivenessDirective` on Android),
/// reported via the `onActiveLivenessDirectiveChanged` callback during
/// active liveness.
enum ActiveLivenessDirective {
  turnHeadLeft,
  turnHeadRight,
  turnHeadUp,
  turnHeadDown,
  ...
  static ActiveLivenessDirective fromString(String? value) {
    for (final directive in ActiveLivenessDirective.values) {
      if (directive.name == value) return directive;
    }
    return ActiveLivenessDirective.unknown;
  }
}
...
/// Result of a demographics estimation request (age and gender from a selfie)
class DemographicsResult {
  ...
  factory DemographicsResult.fromMap(Map<String, dynamic> map) {
    return DemographicsResult(
      estimatedAge: map['estimatedAge'] is int
          ? map['estimatedAge'] as int
          : int.tryParse('${map['estimatedAge']}'),
      estimatedGender: map['estimatedGender'] as String?,
    );
  }
