...
class LivenessFlutter {
  ...
  /// Set callback for real-time face recognition feedback (FaceRecognizer
  /// interface's `onFaceDirectiveChanged`, added in native SDK v26.3.0814).
  ///
  /// [callback] - Function to be called whenever the face workflow state
  /// changes (e.g. face off-center, too big/small, angled, mask detected).
  static Future<void> setOnFaceDirectiveChangedCallback(
      Function(FaceDirective) callback) {
    return _platform.setOnFaceDirectiveChangedCallback(callback);
  }

  /// Set callback for real-time active liveness feedback
  /// (ActiveLivenessOperator interface's `onActiveLivenessDirectiveChanged`,
  /// added in native SDK v26.3.0814).
  ///
  /// [callback] - Function to be called whenever the active liveness
  /// workflow state changes (e.g. turn head left/right/up/down, blink, smile).
  static Future<void> setOnActiveLivenessDirectiveChangedCallback(
      Function(ActiveLivenessDirective) callback) {
    return _platform.setOnActiveLivenessDirectiveChangedCallback(callback);
  }
  ...
  /// Estimate demographics (age and gender) from a selfie photo.
  ///
  /// Capture the selfie with [startSelfieCapture] first, then pass the
  /// base64-encoded image here. The estimation runs on the server.
  ///
  /// [serverURL] - The server endpoint URL.
  /// [transactionID] - The transaction ID.
  /// [imageBase64] - Base64-encoded selfie image.
  ///
  /// Returns a [DemographicsResult] with the estimated age and gender.
  static Future<DemographicsResult> estimateDemographics({
    required String serverURL,
    required String transactionID,
    required String imageBase64,
  }) {
    return _platform.estimateDemographics(
      serverURL: serverURL,
      transactionID: transactionID,
