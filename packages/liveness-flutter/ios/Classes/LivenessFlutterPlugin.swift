...
public class LivenessFlutterPlugin: NSObject, FlutterPlugin {
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "estimateDemographics":
      estimateDemographics(call: call, result: result)
    default:
  ...
  private func estimateDemographics(call: FlutterMethodCall, result: @escaping FlutterResult) {
    #if canImport(UdentifyFACE)
    guard let args = call.arguments as? [String: Any],
      ...
      result(FlutterError(code: "INVALID_ARGUMENTS",
                          message: "serverURL, transactionID and a valid base64 imageBase64 are required",
                          details: nil))
      return
    }
      ...
      IDCameraController.performAgeEstimation(
        ...
        DispatchQueue.main.async {
          if age == nil && gender == nil {
            result(FlutterError(code: "DEMOGRAPHICS_ERROR",
                                message: "Demographics estimation failed",
                                details: nil))
          } else {
    ...
    #else
    result(FlutterError(code: "SDK_NOT_AVAILABLE",
                        message: "UdentifyFACE framework is not available",
                        details: nil))
    #endif
  }
  ...
  private func registerUserWithPhoto(call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    guard let arguments = call.arguments as? [String: Any],
          let serverURL = arguments["serverURL"] as? String,
          let transactionId = arguments["transactionID"] as? String,
          let photoBase64 = arguments["base64Image"] as? String,
          let photoData = Data(base64Encoded: photoBase64),
          let photo = UIImage(data: photoData) else {
      result(FlutterError(code: "INVALID_ARGUMENTS", message: "Missing required arguments: serverURL, transactionID, base64Image", details: nil))
      return
    }
    ...
    DispatchQueue.main.async { [weak self] in
      IDCameraController.instantiate(
        ...
        controller.register(forImage: photo) { [weak self] response in
          DispatchQueue.main.async {
            guard let self = self else { return }
            result(self.createPhotoBasedResultMap(faceIDResult: response, failureMessage: "Face registration failed", successMessage: "Face registration completed"))
          }
        }
      }
    }
  ...
  private func authenticateUserWithPhoto(call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    DispatchQueue.main.async { [weak self] in
      IDCameraController.instantiate(
        ...
        controller.authenticate(forImage: photo) { [weak self] response in
          DispatchQueue.main.async {
            guard let self = self else { return }
            result(self.createPhotoBasedResultMap(faceIDResult: response, failureMessage: "Face authentication failed", successMessage: "Face authentication completed"))
          }
        }
      }
    }
    ...
    DispatchQueue.main.async {
      IDCameraController.addUserToList(
        ...
        DispatchQueue.main.async {
          ...
          // ListResponseData's properties aren't exposed as public getters in the
          // UdentifyFACE module, but it does conform to Codable, so round-trip it
          // through JSON to recover its real field values instead of fabricating them.
          var data: [String: Any] = [:]
        ...
        DispatchQueue.main.async {
          if let error = error {
            result([
              "success": false,
              "error": [
                "code": "ERR_DELETE_USER_FROM_LIST",
                "message": error.localizedDescription
              ]
            ])
            return
          ...
          // NOTE: UdentifyFACE's ListUser type exposes no public properties (and isn't
          // Codable) in this SDK version, so userID/matchScore/registrationTransactionID
          // can't be read back from the response on iOS. We echo the request's own
          // transactionID/listName (genuine, not fabricated) and report success based on
          // whether the SDK returned a user with no error.
          result([
  ...
  // MARK: - Helper Methods for Photo-Based Register/Authenticate

  // Builds the same {"status", "faceIDMessage"} shape the camera-based flows use so
  // Dart's FaceRecognitionResult.fromMap / FaceIDMessage.fromMap can parse the real
  // verified/matchScore/transactionID data returned by controller.register(forImage:)
  // and controller.authenticate(forImage:) instead of the ad-hoc "result"/"status" map
  // this used to return.
  private func createPhotoBasedResultMap(faceIDResult: FaceIDResult, failureMessage: String, successMessage: String) -> [String: Any] {
    let isFailed = faceIDResult.error != nil || !faceIDResult.verified
    ...
    if let referencePhoto = faceIDResult.referencePhoto,
      ...
      faceIDResultDict["referencePhotoBase64"] = photoData.base64EncodedString()
    }
    ...
    let faceIDMessage: [String: Any] = [
      "success": !isFailed,
      "isFailed": isFailed,
      "message": isFailed ? failureMessage : successMessage,
      "faceIDResult": faceIDResultDict
    ]
...
extension LivenessFlutterPlugin: IDCameraControllerDelegate {
  public func cameraController(image: UIImage) {
    DispatchQueue.main.async { [weak self] in
      ...
      // Mirrors Android's FaceRecognizer.onPhotoTaken, fired alongside onSelfieTaken so
      // Dart's setOnPhotoTakenCallback fires the same way on both platforms.
  ...
  public func onFaceDirectiveChanged(directive: FaceDirective) {
    DispatchQueue.main.async { [weak self] in
      self?.channel?.invokeMethod("onFaceDirectiveChanged", arguments: ["directive": "\(directive)"])
    }
  }
}
...
extension LivenessFlutterPlugin: ActiveCameraControllerDelegate {
  ...
  public func onVideoTaken() {
    ...
    DispatchQueue.main.async { [weak self] in
      // Mirrors Android's onPhotoTaken, fired when the active/hybrid liveness camera
      // finishes capturing and before the result comes back from the server. This is
      // the iOS equivalent of the 2 Android call sites in startActiveLiveness/
      // startHybridLiveness's FaceRecognizer proxies.
      self?.channel?.invokeMethod("onPhotoTaken", arguments: nil)
  ...
  public func onActiveLivenessDirectiveChanged(directive: ActiveLivenessDirective) {
    DispatchQueue.main.async { [weak self] in
      self?.channel?.invokeMethod("onActiveLivenessDirectiveChanged", arguments: ["directive": "\(directive)"])
    }
  }
}
#endif