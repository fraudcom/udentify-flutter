...
A Flutter plugin for reading MRZ (Machine Readable Zone) from documents using Udentify SDK.
  ...
  # Flutter.framework does not contain a i386 slice, and the vendored
  # GPUImage/UdentifyMRZ/TesseractOCRSDKiOS frameworks only ship an
  # ios-x86_64-simulator slice (no arm64-simulator slice), so arm64 must
  # also be excluded for simulator builds on Apple Silicon Macs.
