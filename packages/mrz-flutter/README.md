...
1. **Add MRZ AAR files** to `android/libs/` directory:
   ...
   - `commons-26.3.0814.aar` (from Udentify SDK)
...
dependencies {
    ...
    // Local AAR files
    implementation fileTree(dir: '../../../packages/mrz-flutter/android/libs', include: ['*.aar'])
}
```

3. (Alternative) **Maven dependency** (requires GitHub Packages access): add the GitHub repo to your project-level `build.gradle` and include `com.fraud.udentify.android.sdk:mrz:26.3.0814` in app-level dependencies. Set `GITHUB_ACTOR` and `GITHUB_TOKEN` env vars.
