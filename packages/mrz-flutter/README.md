...
1. **Add MRZ AAR files** to `android/libs/` directory:
   ...
   - `commons-26.1.3.aar` (from Udentify SDK)
...
3. (Alternative) **Maven dependency** (requires GitHub Packages access): add the GitHub repo to your project-level `build.gradle` and include `com.fraud.udentify.android.sdk:mrz:26.1.3` in app-level dependencies. Set `GITHUB_ACTOR` and `GITHUB_TOKEN` env vars.
