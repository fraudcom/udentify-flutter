...
Supports real-time video communication, camera/microphone controls, and UI customization.
  ...
  # LiveKit dependency for video calling.
  # Pinned to >= 2.14 (rather than the looser '~> 2.0') because CocoaPods'
  # default trunk source only has LiveKitClient up to 2.0.18 (which predates
  # ConnectionState.disconnecting, used by VCCameraController.swift) and
  # CocoaPods stops at the first source satisfying the constraint — so a
  # loose '~> 2.0' silently resolves from trunk instead of the newer
  # livekit-podspecs source declared in the app's Podfile.
  s.dependency 'LiveKitClient', '>= 2.13', '< 2.14'
