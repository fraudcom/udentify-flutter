    ...
    */
    ...
    fun startVideoCall(activity: FragmentActivity): Boolean {
        return try {
            ...
            // Re-apply any configuration that was cached before the fragment
            // existed (e.g. setVideoCallConfig called before startVideoCall).
            applyConfigToFragment()
            true
        } catch (e: Exception) {
            currentStatus = "failed"
    ...
    fun didChangeUserState(userState: Any) {
        val stateString = userState.toString()
        android.util.Log.d("VideoCallOperator", "didChangeUserState: $stateString")
        notifyUserStateChanged(stateString)
    }

    fun didChangeParticipantState(participantState: Any) {
        val stateString = participantState.toString()
        android.util.Log.d("VideoCallOperator", "didChangeParticipantState: $stateString")
        notifyParticipantStateChanged(stateString)
    }
    ...
    // Applies the currently cached configuration fields to the Udentify
    // VideoCallFragment, if one is available yet. Safe to call multiple
    // times: once when setConfig() is called (in case the fragment already
    // exists), and again once the fragment becomes available after
    // startVideoCall() runs, so config set before OR after startVideoCall
    // ends up applied either way.
    private fun applyConfigToFragment() {
        try {
    ...
    private fun notifyUserStateChanged(state: String) {
        try {
            channel?.invokeMethod("onUserStateChanged", mapOf("state" to state))
        } catch (e: Exception) {
            // Handle error silently
        }
    }

    private fun notifyParticipantStateChanged(state: String) {
        try {
            channel?.invokeMethod("onParticipantStateChanged", mapOf("state" to state))
        } catch (e: Exception) {
            // Handle error silently
        }
    }
