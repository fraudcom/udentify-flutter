...
public class UdentifyCoreFlutterPlugin: NSObject, FlutterPlugin {
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "fetchTransactionProperties":
            fetchTransactionProperties(call: call, result: result)
        case "resetTransactionProperties":
            UdentifySettingsProvider.resetTransactionProperties()
            result(nil)
        case "setDeviceInfoSubPath":
            let subPath = (call.arguments as? [String: Any])?["subPath"] as? String
            UdentifySettingsProvider.setDeviceInfoSubPath(subPath)
            debugPrint("UdentifyCoreFlutterPlugin - setDeviceInfoSubPath applied, readback: \(UdentifySettingsProvider.getDeviceInfoSubPath() ?? "nil")")
            result(nil)
        default:
    ...
    /**
     * Fetch and cache transaction properties (e.g. U18) so SDK flows started
     * afterwards can read them. Must be called after obtaining the transaction ID
     * and before opening the OCR camera.
     */
    private func fetchTransactionProperties(call: FlutterMethodCall, result: @escaping FlutterResult) {
        guard let args = call.arguments as? [String: Any],
              let serverURL = args["serverURL"] as? String,
              let transactionID = args["transactionID"] as? String else {
            result(FlutterError(code: "INVALID_ARGUMENTS", message: "serverURL and transactionID are required", details: nil))
            return
        }
        // UdentifyCommons.UdentifySettingsProvider.fetchTransactionProperties(serverUrl:transactionId:) is a
        // fire-and-forget void call in the vendored native SDK (see UdentifyCommons.swiftinterface) — it exposes
        // no completion closure, delegate, or notification to learn when/if the fetched names become available,
        // unlike e.g. instantiateServerBasedLocalization which does take a completion handler. There is a
        // synchronous UdentifySettingsProvider.getTransactionProperties() getter, but calling it immediately here
        // would race the in-flight network request and could return stale/empty data, which would be worse than
        // being explicit. Rather than fabricate an async bridge the SDK doesn't provide, we trigger the fetch and
        // report back an empty list so Dart callers on iOS don't mistake this for "no properties returned".
