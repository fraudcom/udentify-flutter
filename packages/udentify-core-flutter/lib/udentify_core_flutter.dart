...
class UdentifyCoreFlutter {
  ...
  /// Fetch and cache the transaction properties (e.g. U18) for the given
  /// transaction so SDK flows started afterwards can read them.
  ///
  /// Must be called after obtaining the transaction ID and before opening
  /// the OCR camera; calling it too late may cause the SDK to miss the
  /// U18 flag. On Android returns the fetched property names.
  ///
  /// On iOS the underlying native SDK call
  /// (`UdentifySettingsProvider.fetchTransactionProperties`) is fire-and-forget
  /// and exposes no completion/delegate callback to observe when the fetch
  /// finishes, so the fetch is still triggered but this always resolves with
  /// an empty list on iOS rather than the actual property names.
  ///
  /// ```dart
  /// final txId = await getTransactionId();
  /// await UdentifyCoreFlutter.fetchTransactionProperties(serverUrl, txId);
  /// // proceed to open the OCR camera
  /// ```
  static Future<List<String>?> fetchTransactionProperties(
      String serverURL, String transactionID) {
    return _platform.fetchTransactionProperties(serverURL, transactionID);
  }

  /// Reset any cached transaction properties from a previous session.
  ///
  /// Call before starting a new transaction (before getting the
  /// transaction ID) to prevent stale data from a previous session.
  static Future<void> resetTransactionProperties() {
    return _platform.resetTransactionProperties();
  }

  /// Set a custom sub-path for the device-info reporting endpoint the SDK
  /// calls in the background (`UdentifySettingsProvider.setDeviceInfoSubPath`
  /// on both native platforms).
  ///
  /// Example:
  /// ```dart
  /// await UdentifyCoreFlutter.setDeviceInfoSubPath('/custom/device-info');
  /// ```
  ///
  /// [subPath] - The sub-path to use, or null to reset to the SDK's default.
  static Future<void> setDeviceInfoSubPath(String? subPath) {
    return _platform.setDeviceInfoSubPath(subPath);
  }
}
