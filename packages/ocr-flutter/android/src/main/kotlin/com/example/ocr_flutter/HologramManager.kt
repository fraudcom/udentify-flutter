...
class HologramManager(
    ...
    fun startHologramCamera(call: MethodCall, result: Result, activity: Activity?) {
        try {
            ...
            val noFlashDuration = call.argument<Int>("noFlashDuration")
            val flashDuration = call.argument<Int>("flashDuration")
            val totalDuration = call.argument<Int>("totalDuration")
            val bitrate = call.argument<Double>("bitrate")
            val mappedCountryCode = CountryCodeMapper.toCountryCode(countryStr ?: "TUR")

            // Create credentials for hologram (must be fully populated: the SDK derives
            // the card mask image from docType/countryCode/placeholderTemplate)
            val builder = CardRecognizerCredentials.Builder()
                .serverURL(serverURL)
                .transactionID(transactionID)
                .userID(userID ?: Date().time.toString())
            ...
            if (noFlashDuration != null) builder.hologramNoFlashDuration(noFlashDuration)
            if (flashDuration != null) builder.hologramFlashDuration(flashDuration)
            if (totalDuration != null) builder.hologramTotalDuration(totalDuration)
            if (bitrate != null) builder.hologramBitrate(bitrate.toFloat())
    ...
    override fun onHologramDirectiveChanged(directive: HologramDirective) {
        Log.d(TAG, "HologramManager - Hologram Directive changed: ${directive.name}")
        Handler(Looper.getMainLooper()).post {
            channel.invokeMethod("onHologramDirectiveChanged", mapOf(
