...
class OcrProcessor(
    ...
    fun performOCR(call: MethodCall, result: Result, activity: Activity?) {
        ...
        try {
            ...
            // processOCR() has no return value and no listener param - it delivers its outcome
            // through the CardRecognizer interface passed into the constructor above
            // (ocrCameraManager), via onResult()/onFailure(). Register this call's Result there
            // so those callbacks resolve it directly instead of firing the onOCRSuccess/
            // onOCRFailure channel events, which this Future has nothing to do with.
            ocrCameraManager.setDirectOcrResult(result)

            // Start OCR processing
            try {
                cardRecognizerObject.processOCR()
            } catch (e: Exception) {
                ocrCameraManager.setDirectOcrResult(null)
                throw e
            }
