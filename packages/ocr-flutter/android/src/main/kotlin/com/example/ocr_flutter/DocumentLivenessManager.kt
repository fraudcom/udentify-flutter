...
class DocumentLivenessManager(
    ...
    fun performOCRAndDocumentLiveness(call: MethodCall, result: Result, activity: Activity?) {
        try {
            ...
            // Unlike performDocumentLiveness(), the SDK's performOCRAndDocumentLiveness(String,
            // String, String, DocumentType) overload (confirmed by decompiling
            // ocr-26.3.0814.aar - it is the only non-empty overload available) takes no
            // DocumentLivenessListener parameter and no back-side-photo parameter. It instead
            // delivers its result through the CardRecognizer interface passed into
            // CardRecognizerObject's constructor (the same interface OcrCameraManager
            // implements for the live camera flow), calling back through
            // didFinishOcrAndDocumentLivenessCheck()/onFailure(). The previous single-parameter
            // CardRecognizerObject(currentActivity) constructor never set that interface, so the
            // SDK had nowhere to deliver the result and currentResult was never resolved.
            //
            // Build a minimal CardRecognizer here (rather than reusing ocrCameraManager) so the
            // callback bridges straight into documentLivenessListener - and therefore
            // currentResult - exactly like performDocumentLiveness does, instead of firing the
            // onOCRAndDocumentLivenessResult/Failure channel events used by the live camera flow.
            ...
            val cardRecognizer = object : CardRecognizer {
                ...
                override fun getCredentials(): CardRecognizerCredentials = credentials

                override fun onFailure(s: String) {
                    documentLivenessListener.errorResponse(s)
                }

                override fun didFinishOcrAndDocumentLivenessCheck(response: OCRAndDocumentLivenessResponse) {
                    if (response.isFailed()) {
                        documentLivenessListener.errorResponse(
                            response.errorCode ?: "OCR and document liveness check failed"
                        )
                    } else {
                        documentLivenessListener.successResponse(response)
                    }
                }
            ...
            // Create CardRecognizerObject with the images and the CardRecognizer above so the
            // SDK's callback has somewhere to deliver the result (front image is passed both here
            // and to the call below since the call itself only accepts the front-side photo).
            val cardRecognizerObject = CardRecognizerObject(
                cardRecognizer,
                currentActivity,
                frontSidePhoto ?: "",
