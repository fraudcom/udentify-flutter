...
public class OcrFlutterPlugin: NSObject, @preconcurrency FlutterPlugin, UIImagePickerControllerDelegate, UINavigationControllerDelegate, @unchecked Sendable {
    ...
    // Take photo result handler
    private var takePhotoResult: FlutterResult?

    // State captured from the most recent startOCRCamera call so onDocumentScan can, when
    // livenessMode was requested, automatically run the OCR+document-liveness check once
    // capture completes (mirroring Android's isDocumentLivenessActive credential flag - iOS's
    // OCRCameraViewController.instantiate(withApiCallDisabled:...) has no equivalent flag and
    // never calls the API itself, so the plugin has to trigger it explicitly).
    private var cameraLivenessModeActive: Bool = false
    private var cameraServerURL: String?
    private var cameraTransactionID: String?
    private var cameraCountry: Country?
    private var cameraDocumentType: OCRDocumentType?
    private var cameraDocumentSide: OCRDocumentSide = .bothSides
    private var didTriggerCameraLivenessCheck: Bool = false
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "performIQA":
            handlePerformIQA(call, result: result)
        case "takePhoto":
    ...
    private func handleStartOCRCamera(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        // Android forces isDocumentLivenessActive(true) on the SDK credentials when this is
        // set, so its camera flow completes through the combined OCR+liveness callback
        // instead of the plain OCR one. iOS's OCRCameraViewController has no equivalent
        // credential flag (confirmed against UdentifyOCR's swiftinterface - instantiate(
        // withApiCallDisabled:...) never calls the API itself either way), so this is read
        // here and used to drive the same branching from onDocumentScan once capture completes.
        let livenessMode = args["livenessMode"] as? Bool ?? false

        // Refresh SDK settings so the crop ratio reaches OCRConfigs before the camera opens
        if let rawPhotoCropRatio = args["rawPhotoCropRatio"] as? Double {
            var mergedConfig = uiConfiguration ?? [:]
            mergedConfig["rawPhotoCropRatio"] = rawPhotoCropRatio
            uiConfiguration = mergedConfig
            let bundle = Bundle(for: OcrFlutterPlugin.self)
            let settings = CustomOCRSettings(localizationBundle: bundle, uiConfig: mergedConfig)
            OCRSettingsProvider.getInstance().currentSettings = settings
            debugPrint("OcrFlutterPlugin - rawPhotoCropRatio applied to OCRConfigs: \(settings.configs.rawPhotoCropRatio ?? -1)")
        }
        ...
        DispatchQueue.main.async { [weak self, capturedCountry, threadSafeResult] in
            ...
            strongSelf.cameraLivenessModeActive = livenessMode
            strongSelf.cameraServerURL = serverURL
            strongSelf.cameraTransactionID = transactionID
            strongSelf.cameraCountry = capturedCountry
            strongSelf.cameraDocumentType = documentType
            strongSelf.cameraDocumentSide = documentSide
            strongSelf.didTriggerCameraLivenessCheck = false
    ...
    /// Accepts raw base64 (the documented format) and tolerates an optional
    /// "data:image/...;base64," data-URI prefix. Returns nil for null/empty/invalid input.
    private func decodeBase64Image(_ base64String: String?) -> UIImage? {
        // The camera-capture flow sends the sentinel "IMAGE_PATH_STORED" instead of real
        // image data; treat it (and null/empty) as "no provided photo" so performOCR falls
        // back to the natively-stored capture.
        guard let base64String = base64String, !base64String.isEmpty,
              base64String != "IMAGE_PATH_STORED" else { return nil }
        let cleaned: String
    ...
    private func handlePerformOCR(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        // Externally-provided photos (base64-encoded JPEG, no data-URI prefix).
        // When supplied, these take precedence over camera-captured stored images.
        let providedFrontPhoto = args["frontSidePhoto"] as? String
        let providedBackPhoto = args["backSidePhoto"] as? String
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            let providedFrontImage = self.decodeBase64Image(providedFrontPhoto)
            let providedBackImage = self.decodeBase64Image(providedBackPhoto)

            let payload: DocumentScanPayload
            if providedFrontImage != nil || providedBackImage != nil {
                debugPrint("OcrFlutterPlugin - Using externally-provided photos for performOCR")
                payload = .images(front: providedFrontImage, back: providedBackImage)
            } else if self.lastCapturedFrontImagePath != nil || self.lastCapturedBackImagePath != nil {
    ...
    private func handleStartHologramCamera(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        let noFlashDuration = args["noFlashDuration"] as? Double
        let flashDuration = args["flashDuration"] as? Double
        let totalDuration = args["totalDuration"] as? Double
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            strongSelf.hologramCameraController = HologramCameraViewControllerV2.instantiate(
                delegate: strongSelf,
                serverURL: serverURL,
                transactionID: transactionID,
                country: country,
                noFlashDuration: noFlashDuration ?? 2,
                flashDuration: flashDuration ?? 3,
    ...
    private func handleUploadHologramVideo(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        guard let videoUrl = videoUrls.first else {
            threadSafeResult.send(FlutterError(code: "INVALID_ARGUMENTS", message: "No valid video URL provided", details: nil))
            return
        }
    ...
    private func handlePerformDocumentLiveness(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        // Externally-provided photos (base64-encoded JPEG, no data-URI prefix). When supplied,
        // these take precedence over camera-captured stored images, matching performOCR.
        let providedFrontPhoto = args["frontSidePhoto"] as? String
        let providedBackPhoto = args["backSidePhoto"] as? String
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            let payload: DocumentScanPayload
            if providedFrontImage != nil || providedBackImage != nil {
                debugPrint("OcrFlutterPlugin - Using externally-provided photos for performDocumentLiveness")
                payload = .images(front: providedFrontImage, back: providedBackImage)
            } else if self.lastCapturedFrontImagePath != nil || self.lastCapturedBackImagePath != nil {
    ...
    private func handlePerformOCRAndDocumentLiveness(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            let payload: DocumentScanPayload
            if providedFrontImage != nil || providedBackImage != nil {
                debugPrint("OcrFlutterPlugin - Using externally-provided photos for performOCRAndDocumentLiveness")
                payload = .images(front: providedFrontImage, back: providedBackImage)
            } else if self.lastCapturedFrontImagePath != nil || self.lastCapturedBackImagePath != nil {
    ...
    private func convertOCRResponseToDict(_ response: OCRResponse, transactionID: String) -> [String: any Sendable] {
        ...
        case .idCard(let idCardResponse):
            ...
            // Coalesce so the keys stay present when the server returns no
            // native-script data (nil values would drop the keys entirely)
            idCardDict["nativeFirstName"] = idCardResponse.nativeFirstName ?? ""
            idCardDict["nativeLastName"] = idCardResponse.nativeLastName ?? ""
            idCardDict["nativeGender"] = idCardResponse.nativeGender ?? ""
            if let faceImage = idCardResponse.faceImage,
               let imageData = faceImage.jpegData(compressionQuality: 0.8) {
        ...
        case .passport(let passportResponse):
            ...
            if let faceImage = passportResponse.faceImage {
                let imageData = faceImage.jpegData(compressionQuality: 0.8)
                extractedData["faceImage"] = imageData?.base64EncodedString() ?? ""
            }
            ...
            var passportDict: [String: any Sendable] = [:]
            passportDict["documentType"] = passportResponse.documentType
            passportDict["countryCode"] = passportResponse.countryCode
            passportDict["documentID"] = passportResponse.documentID
            passportDict["firstName"] = passportResponse.firstName
            passportDict["lastName"] = passportResponse.lastName
            passportDict["identityNo"] = passportResponse.identityNo
            passportDict["birthDate"] = passportResponse.birthDate
            passportDict["expiryDate"] = passportResponse.expiryDate
            passportDict["nationality"] = passportResponse.nationality
            passportDict["gender"] = passportResponse.gender
            passportDict["isOCRDocumentExpired"] = passportResponse.isOCRDocumentExpired
            passportDict["isOCRIDValid"] = passportResponse.isOCRIDValid
            passportDict["hasOCRSignature"] = passportResponse.hasOCRSignature
            passportDict["mrzString"] = passportResponse.mrzString
            passportDict["nativeFirstName"] = passportResponse.nativeFirstName ?? ""
            passportDict["nativeLastName"] = passportResponse.nativeLastName ?? ""
            passportDict["nativeGender"] = passportResponse.nativeGender ?? ""
            if let faceImage = passportResponse.faceImage,
               let imageData = faceImage.jpegData(compressionQuality: 0.8) {
                passportDict["faceImage"] = imageData.base64EncodedString()
            }
            resultDict["passportResponse"] = passportDict
        @unknown default:
            fatalError("")
        }
    ...
    private func handlePerformIQA(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        guard let args = call.arguments as? [String: Any] else {
            result(FlutterError(code: "IQA_ERROR", message: "Invalid arguments", details: nil))
            return
        }
        ...
        let documentType = args["documentType"] as? String ?? "ID_CARD"
        let documentSide = args["documentSide"] as? String ?? "FRONT"
        let country = args["country"] as? String ?? "TUR"

        guard !imageBase64.isEmpty,
              let imageData = Data(base64Encoded: imageBase64),
              let image = UIImage(data: imageData) else {
            result(FlutterError(code: "IQA_ERROR", message: "Invalid or empty image base64 data", details: nil))
            return
        }

        let ocrDocumentType: OCRDocumentType
        switch documentType.uppercased() {
        case "ID_CARD": ocrDocumentType = .ID_CARD
        case "PASSPORT": ocrDocumentType = .PASSPORT
        case "DRIVER_LICENSE", "DRIVE_LICENCE": ocrDocumentType = .DRIVE_LICENCE
        default: ocrDocumentType = .ID_CARD
        }

        let ocrDocumentSide: OCRDocumentSide
        switch documentSide.uppercased() {
        case "FRONT", "FRONTSIDE": ocrDocumentSide = .frontSide
        case "BACK", "BACKSIDE": ocrDocumentSide = .backSide
        default: ocrDocumentSide = .frontSide
        }

        let ocrCountry: UdentifyCommons.Country = .TUR
        ...
        OCRCameraViewController.performIQA(
            ...
            case .success(let iqaResponse):
                let resultDict: [String: Any] = [
                    "qualified": iqaResponse.qualified,
                    "displayMessage": iqaResponse.displayMessage ?? "",
                    "rawMessage": iqaResponse.rawMessage ?? "",
                    "documentSide": "\(iqaResponse.documentSide)",
                    "timestamp": Date().timeIntervalSince1970
                ]
                result(resultDict)
            case .failure(let error):
                result(FlutterError(code: "IQA_ERROR", message: error.localizedDescription, details: nil))
            }
        }
    }
    ...
    private func handleTakePhoto(_ result: @escaping FlutterResult) {
        DispatchQueue.main.async {
            guard UIImagePickerController.isSourceTypeAvailable(.camera) else {
                result(FlutterError(code: "TAKE_PHOTO_ERROR", message: "Camera not available", details: nil))
                return
            }

            guard let topVC = self.topViewController() else {
                result(FlutterError(code: "TAKE_PHOTO_ERROR", message: "Unable to find view controller", details: nil))
                return
            }

            self.takePhotoResult = result
        ...
        picker.dismiss(animated: true) {
            if let image = info[.originalImage] as? UIImage,
               let data = image.jpegData(compressionQuality: 0.8) {
                ...
                self.takePhotoResult?(base64)
            } else {
                self.takePhotoResult?(FlutterError(code: "TAKE_PHOTO_ERROR", message: "Failed to capture photo", details: nil))
            }
            self.takePhotoResult = nil
        }
    }

    public func imagePickerControllerDidCancel(_ picker: UIImagePickerController) {
        picker.dismiss(animated: true) {
            self.takePhotoResult?(FlutterError(code: "TAKE_PHOTO_ERROR", message: "Photo capture cancelled", details: nil))
            self.takePhotoResult = nil
        }
    }
}
...
extension OcrFlutterPlugin: @preconcurrency OCRCameraControllerDelegate, @preconcurrency HologramCameraControllerDelegate {
    
    public func onDocumentScan(for side: UdentifyOCR.OCRDocumentSide, payload: UdentifyOCR.DocumentScanPayload, croppedImages: (front: UIImage?, back: UIImage?)) {
        ...
        // livenessMode requests the combined OCR+document-liveness flow (see
        // handleStartOCRCamera). The camera controller itself never calls that API (there is
        // no SDK flag to activate it, unlike Android's isDocumentLivenessActive), so trigger it
        // here once every side the configured documentSide needs has been captured, and route
        // the outcome through onOCRAndDocumentLivenessResult/Failure instead of the plain
        // onOCRSuccess/onOCRFailure events.
        if cameraLivenessModeActive && !didTriggerCameraLivenessCheck {
            let frontReady = lastCapturedFrontImage != nil || lastCapturedFrontImagePath != nil
            let backReady = lastCapturedBackImage != nil || lastCapturedBackImagePath != nil
            let captureComplete: Bool
            switch cameraDocumentSide {
            case .frontSide:
                captureComplete = frontReady
            case .backSide:
                captureComplete = backReady
            case .bothSides:
                captureComplete = frontReady && backReady
            @unknown default:
                captureComplete = frontReady || backReady
            }

            if captureComplete {
                didTriggerCameraLivenessCheck = true
                triggerCameraDocumentLiveness()
            }
        }
    }

    /// Runs the combined OCR+document-liveness check against the images captured by the OCR
    /// camera when livenessMode was requested, since OCRCameraViewController never makes that
    /// call itself. Mirrors handlePerformOCRAndDocumentLiveness's own call below.
    private func triggerCameraDocumentLiveness() {
        guard let serverURL = cameraServerURL,
              let transactionID = cameraTransactionID,
              let documentType = cameraDocumentType else {
            return
        }

        let payload: DocumentScanPayload
        if lastCapturedFrontImagePath != nil || lastCapturedBackImagePath != nil {
            payload = .imagePaths(front: lastCapturedFrontImagePath, back: lastCapturedBackImagePath)
        } else {
            payload = .images(front: lastCapturedFrontImage, back: lastCapturedBackImage)
        }

        let country = cameraCountry
        ...
        OCRCameraViewController.performOCRAndDocumentLiveness(
            ...
            DispatchQueue.main.async {
                ...
                if response.isFailed {
                    self.channel?.invokeMethod("onOCRAndDocumentLivenessFailure", arguments: ["error": "OCR and document liveness check failed"])
                } else {
                    let responseDict = self.convertDocumentLivenessResponseToDict(response, transactionID: transactionID)
                    self.channel?.invokeMethod("onOCRAndDocumentLivenessResult", arguments: responseDict)
                }
            }
        }
    }
    
    public func onIqaResult(for side: UdentifyOCR.OCRDocumentSide, iqaFeedback: UdentifyOCR.IQAFeedback, iqaResponse: UdentifyOCR.IQAResponse?) {
        ...
        case .modelFailed:
            ...
            iqaResult["message"] = "IQA model failed to process the image"
        case .other:
    ...
    public func onOCRDirectiveChanged(directive: UdentifyOCR.OCRDirective) {
        debugPrint("OcrFlutterPlugin - onOCRDirectiveChanged: \(directive.rawValue)")
        DispatchQueue.main.async { [weak self] in
            self?.channel?.invokeMethod("onOCRDirectiveChanged", arguments: [
    ...
    public func onHologramDirectiveChanged(directive: UdentifyOCR.HologramDirective) {
        debugPrint("OcrFlutterPlugin - onHologramDirectiveChanged: \(directive.rawValue)")
        DispatchQueue.main.async { [weak self] in
            self?.channel?.invokeMethod("onHologramDirectiveChanged", arguments: [
                "directive": directive.rawValue,
                "timestamp": Date().timeIntervalSince1970
            ])
        }
    ...
    public func didFinishOcrAndDocumentLivenessCheck(response: OCRAndDocumentLivenessResponse) {
        ...
        DispatchQueue.main.async { [weak self] in
            if response.isFailed {
                self?.channel?.invokeMethod("onOCRAndDocumentLivenessFailure", arguments: ["error": "Document liveness check failed"])
            } else {
