...
The plugin bundles `nfc-26.3.0814.aar` and declares the Udentify SDK as `compileOnly`, so **the host app must make the native SDK available at runtime**. Choose one of the two options below, then add the third-party dependencies and permissions.
...
implementation 'com.fraud.udentify.android.sdk:nfc:26.3.0814'
```
...
- `nfc-26.3.0814.aar` (from `packages/nfc-flutter/android/libs/`)
