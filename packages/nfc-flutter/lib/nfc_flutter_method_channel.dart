...
class MethodChannelNfcFlutter extends NfcFlutterPlatform {
  ...
  Future<NfcPassport?> readPassport(
    ...
    // Always (re)set the channel-level handler for this call, so a handler
    // left behind by a previous readPassport call can never leak into this
    // one. Pass null when no onProgress is given, so any stale handler is
    // cleared up front.
    if (onProgress != null) {
    ...
    try {
      ...
      // This call's handler no longer applies once the read has settled,
