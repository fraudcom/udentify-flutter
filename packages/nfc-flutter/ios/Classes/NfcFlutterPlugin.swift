...
public class NfcFlutterPlugin: NSObject, FlutterPlugin {
    ...
    private var currentReadResult: ThreadSafeFlutterResult?
#endif
    ...
    private func readPassport(call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        let chipAnticloneModeStr = args["chipAnticloneMode"] as? String
        let chipAnticloneMode: ChipAnticloneMode
        switch chipAnticloneModeStr {
        case "none":
            chipAnticloneMode = .none
        case "requireCA":
            chipAnticloneMode = .requireCA
        case "requireAA":
            chipAnticloneMode = .requireAA
        default:
            chipAnticloneMode = .auto
        }
        let isPassiveAuthEnabled = args["isPassiveAuthenticationEnabled"] as? Bool ?? true

        // Track the result for this in-flight read so cancelReading() can
        // resolve it explicitly (mirrors Android's guaranteed resolution).
        self.currentReadResult = threadSafeResult
        ...
        self.nfcReader?.read { [weak self, threadSafeResult] passport, error, progress in
            DispatchQueue.main.async {
                ...
                if let passport = passport {
                    ...
                    strongSelf.currentReadResult = nil
...
#if canImport(UdentifyNFC)
        // Explicitly resolve the pending readPassport result with CANCELLED,
        // mirroring Android's guaranteed resolution. We don't rely on the
        // vendor SDK re-invoking the original read closure on cancel.
        // ThreadSafeFlutterResult guards against double-completion, so this
        // stays safe even if the vendor SDK's callback also fires afterward.
        if let pendingReadResult = self.currentReadResult {
            pendingReadResult.send(FlutterError(code: "CANCELLED", message: "Reading was cancelled", details: nil))
            self.currentReadResult = nil
        }
        ...
        self.nfcLocator?.locateNFC { [weak self, threadSafeResult] location, error in
            DispatchQueue.main.async {
                ...
                if let error = error {
                    ...
                    threadSafeResult.send(strongSelf.nfcLocationToJSON(location))
                } else {
                    threadSafeResult.send(strongSelf.nfcLocationToJSON(nil))
                }
    ...
    // Builds the same JSON shape Android sends for getNfcLocation
    // ({"success":..., "location":..., "locationString":...}) so Dart's
    // jsonDecode(...) works identically on both platforms. NFCLocation's
    // rawValue (Int) lines up with Android's numeric location codes
    // (unknown=0, frontTop=1, frontCenter=2, frontBottom=3, rearTop=4,
    // rearCenter=5, rearBottom=6).
    private func nfcLocationToJSON(_ location: NFCLocation?) -> String {
        let locationString = location.map { nfcLocationToString($0) } ?? "unknown"
        let locationCode = location?.rawValue ?? 0
        ...
        if let jsonData = try? JSONSerialization.data(withJSONObject: resultMap),
           let jsonString = String(data: jsonData, encoding: .utf8) {
            return jsonString
        }
        return "{\"success\":false,\"location\":0,\"locationString\":\"unknown\"}"
    }
#endif
}
