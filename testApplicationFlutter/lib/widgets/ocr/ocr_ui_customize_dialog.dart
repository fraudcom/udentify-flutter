...
const List<OptionItem> backgroundOpacityOptions = [
  OptionItem(value: '1.0', label: '100% (Opaque)'),
  OptionItem(value: '0.6', label: '60%'),
  OptionItem(value: '0.5', label: '50%'),
  OptionItem(value: '0.3', label: '30%'),
  OptionItem(value: '0.2', label: '20% (Default)'),
  OptionItem(value: '0.1', label: '10%'),
];
...
class OCRUICustomizeDialog extends StatefulWidget {
  ...
  /// The last-applied configuration, if any. When provided, the dialog's
  /// fields are seeded from it instead of the hardcoded defaults, so
  /// reopening the dialog reflects what was actually applied.
  final OCRUIConfig? initialConfig;
...
class _OCRUICustomizeDialogState extends State<OCRUICustomizeDialog> {
  ...
  String _iqaPinkTheme = 'false';

  String _backgroundColor = 'purple';
  String _backgroundOpacity = '0.2';
  ...
  void initState() {
    ...
    _placeholderTemplate = _matchOrDefault(
        placeholderTemplates, config.placeholderTemplate?.value, _placeholderTemplate);
    _orientation = _matchOrDefault(orientations, config.orientation?.value, _orientation);
    if (config.detectionAccuracy != null) {
      _detectionAccuracy = _matchOrDefault(
          detectionAccuracyOptions, config.detectionAccuracy.toString(), _detectionAccuracy);
    }
    if (config.reviewScreenEnabled != null) {
      _reviewScreenEnabled = config.reviewScreenEnabled!.toString();
    }
    _reviewBackgroundColor = _matchOrDefault(
        const [OptionItem(value: '', label: 'Default'), ...backgroundColors],
        config.reviewBackgroundColor,
        _reviewBackgroundColor);
    if (config.iqaEnabled != null) {
      _iqaServiceEnabled = config.iqaEnabled!.toString();
    }
    _iqaPinkTheme = config.iqaScreenStyle != null ? 'true' : 'false';
    if (config.backButtonEnabled != null) {
      _backButtonEnabled = config.backButtonEnabled!.toString();
    }

    final style = config.placeholderContainerStyle;
    if (style != null) {
      _backgroundColor =
          _matchOrDefault(backgroundColors, style.backgroundColor, _backgroundColor);
      _backgroundOpacity = _matchOrDefault(backgroundOpacityOptions,
          style.opacity != null ? _formatNum(style.opacity!) : null, _backgroundOpacity);
      _borderColor = _matchOrDefault(borderColors, style.borderColor, _borderColor);
      _cornerRadius = _matchOrDefault(cornerRadiusOptions,
          style.cornerRadius != null ? _formatNum(style.cornerRadius!) : null, _cornerRadius);
    }

    final reviewBgStyle = config.reviewBackgroundStyle;
    if (reviewBgStyle != null) {
      _reviewBgImageBase64 = reviewBgStyle['imageBase64'] as String? ?? '';
      _reviewBgContentMode = reviewBgStyle['contentMode'] as String? ?? _reviewBgContentMode;
      final opacity = reviewBgStyle['opacity'] as num?;
      if (opacity != null) {
        _reviewBgOpacity = _formatNum(opacity);
      }
      _reviewBgBorderColor = reviewBgStyle['borderColor'] as String? ?? _reviewBgBorderColor;
      final borderWidth = reviewBgStyle['borderWidth'] as num?;
      if (borderWidth != null) {
        _reviewBgBorderWidth = _formatNum(borderWidth);
      }
      final cornerRadius = reviewBgStyle['cornerRadius'] as num?;
      if (cornerRadius != null) {
        _reviewBgCornerRadius = _formatNum(cornerRadius);
      }
    }
  }

  /// Formats a number the way the dropdown option values are written
  /// (`8` not `8.0`), so a round-tripped config still matches one of them.
  String _formatNum(num value) {
    final asInt = value.toInt();
    return asInt == value ? asInt.toString() : value.toString();
  }

  /// Returns [value] only if it's one of [options]' values, otherwise
  /// [fallback] — guards against a DropdownButton receiving a value with no
  /// matching item (which would throw) if a config came from outside this
  /// dialog's own preset set.
  String _matchOrDefault(List<OptionItem> options, String? value, String fallback) {
    if (value == null) return fallback;
    return options.any((o) => o.value == value) ? value : fallback;
  }

  IQAScreenStyle _buildPinkIQAScreenStyle() {
    const pinkColor = '#FF69B4';
    const darkPink = '#FF1493';
    const lightPink = '#FFB6C1';

    return IQAScreenStyle(
      backgroundColor: lightPink,
      titleTextColor: darkPink,
      titleFontSize: 20.0,
      successBanner: IQABannerStyle(
        backgroundColor: lightPink,
        iconColor: darkPink,
        titleColor: darkPink,
        descriptionColor: darkPink,
      ),
      ...
      progressBar: IQAProgressBarStyle(
        backgroundColor: lightPink,
        progressColor: darkPink,
        completionColor: darkPink,
        textColor: '#FFFFFF',
      ),
    );
  }
  ...
  void _handleApplyCustomization() {
    final config = OCRUIConfig(
      ...
      orientation: _orientation == 'vertical'
          ? OCROrientation.vertical
          : OCROrientation.horizontal,
      placeholderContainerStyle: OCRViewStyle(
        backgroundColor: _backgroundColor,
        opacity: double.parse(_backgroundOpacity),
        borderColor: _borderColor,
        cornerRadius: double.parse(_cornerRadius),
      ),
      ...
      iqaScreenStyle: _iqaPinkTheme == 'true' ? _buildPinkIQAScreenStyle() : null,
    );
                  ...
                  children: [
                    ...
                    _buildSection(
                      ...
                      [
                        ...
                        const SizedBox(height: 16),
                        _buildOptionPicker(
                          'IQA Screen Style (iOS)',
                          const [
                            OptionItem(value: 'false', label: 'Default'),
                            OptionItem(value: 'true', label: 'Pink Test Theme'),
                          ],
                          _iqaPinkTheme,
                          (value) => setState(() => _iqaPinkTheme = value),
                        ),
                      ],
                    ),
                    const SizedBox(height: 24),
