...
class SelfieTestingWidget extends StatelessWidget {
  ...
  final VoidCallback? onEstimateDemographics;
    ...
    return Column(
      children: [
        ...
        if (hasCapturedSelfie) ...[
          ...
          // Demographics Estimation Button (requires a captured selfie)
          if (onEstimateDemographics != null) ...[
            const SizedBox(height: 12),
            SizedBox(
              width: double.infinity,
              child: ElevatedButton(
                onPressed: (isLoading || isSelfieCapturing || !hasCapturedSelfie)
                    ? null
                    : onEstimateDemographics,
                style: ElevatedButton.styleFrom(
