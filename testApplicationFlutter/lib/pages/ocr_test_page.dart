...
class _OcrTestPageState extends State<OcrTestPage> {
  ...
  OCRUIConfig? _customDialogConfig;
  ...
  OCRPlaceholderTemplate _placeholderTemplate = OCRPlaceholderTemplate.defaultStyle;
  OCROrientation _orientation = OCROrientation.horizontal;
  ...
  void initState() {
    super.initState();
    _setupOCRCallbacks();
    // Seed the customize dialog with the config that's actually active on
    // start (the current preset's values), so opening it before ever
    // pressing "Apply" shows real values instead of the dialog's own
    // hardcoded literal defaults. Doesn't touch _selectedUIPreset — the
    // preset used for the running OCR view is unaffected until the user
    // actually applies from the dialog.
    _customDialogConfig = _buildUIConfig();
  }
  ...
  void _setupOCRCallbacks() {
    ...
    final callbacks = OCRCallbackSet(
      ...
      onOCRAndDocumentLivenessFailure: (String error) {
        if (!mounted) return;
        print('💥 OCR Test Page: OCR + LIVENESS FAILURE CALLBACK for ${widget.pageId}: $error');

        setState(() {
          _status = '❌ OCR + Document Liveness failed: $error';
          _isProcessing = false;
        });
      },
  ...
  OCRUIConfig _buildUIConfig() {
    switch (_selectedUIPreset) {
      ...
      case OCRConstants.uiPresetCustom:
        return _customDialogConfig ?? _buildCustomConfig();
      default:
                      ...
                      onPressed: () {
                        showDialog(
                          ...
                          builder: (context) => OCRUICustomizeDialog(
                            initialConfig: _customDialogConfig,
                            onApply: (config) async {
                              try {
                                setState(() {
                                  _customDialogConfig = config;
                                  _selectedUIPreset = OCRConstants.uiPresetCustom;
                                });
                                await OcrFlutter.setOCRUIConfig(config);
                                if (mounted) {
