...
class _NewLivenessTestPageRefactoredState extends State<NewLivenessTestPageRefactored> {
  ...
  Future<void> _estimateDemographics() async {
    final selfie = _selfieManager.capturedSelfieImage;
    ...
    final transactionId = _currentTransactionId ??
        await ApiUtils.getTransactionId([AppConstants.moduleFaceLiveness]);
    if (transactionId == null) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Failed to get transaction ID for demographics')),
        );
      }
      return;
    }
    ...
    try {
      final result = await LivenessFlutter.estimateDemographics(
        serverURL: ApiUtils.serverUrl,
        transactionID: transactionId,
        imageBase64: selfie,
      );
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          title: const Text('Demographics Estimation'),
          content: Text(
            'Estimated Age: ${result.estimatedAge ?? 'unknown'}\n'
            'Estimated Gender: ${result.estimatedGender ?? 'unknown'}',
          ),
          actions: [
            TextButton(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Demographics estimation failed: $e')),
      );
    }
  }
        ...
        child: Column(
          ...
          children: [
            ...
            SelfieTestingWidget(
              ...
              onEstimateDemographics: _estimateDemographics,
            ),
            const SizedBox(height: 16),
