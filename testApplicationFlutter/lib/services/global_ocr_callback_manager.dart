...
class GlobalOCRCallbackManager {
  ...
  void _setupNativeCallbacks() {
    ...
    // Setup OCR and document liveness failure callback
    if (callbacks.onOCRAndDocumentLivenessFailure != null) {
      OcrFlutter.setOnOCRAndDocumentLivenessFailureCallback((String error) {
        print('📞 GlobalOCRCallbackManager: OCR+Liveness Failure callback for $_activePageId');
        if (_activePageId != null && _pageCallbacks.containsKey(_activePageId!)) {
          _pageCallbacks[_activePageId!]!.onOCRAndDocumentLivenessFailure?.call(error);
        }
      });
    }
...
class OCRCallbackSet {
  ...
  final Function(String)? onOCRAndDocumentLivenessFailure;
  ...
  const OCRCallbackSet({
    ...
    this.onOCRAndDocumentLivenessFailure,
